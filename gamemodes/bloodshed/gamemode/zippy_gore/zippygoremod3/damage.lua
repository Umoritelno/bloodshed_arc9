...
function ENT:ZippyGoreMod3_DamageRagdoll_Gibbing(dmginfo)
	if not dmginfo or dmginfo:GetDamage() == nil then return end
	...
	if bit.band(self:GetFlags(), FL_DISSOLVING) == FL_DISSOLVING and not ZGM3_CVARS["zippygore3_gib_dissolving_ragdoll"] then return end

	if dmginfo:IsExplosionDamage() then
		dmginfo:ScaleDamage(ZGM3_CVARS["zippygore3_explosion_damage_mult"])
	end
	...
	if dmginfo:IsBulletDamage() then
		dmginfo:ScaleDamage(ZGM3_CVARS["zippygore3_bullet_damage_mult"])
	end

	if dmginfo:GetInflictor() and dmginfo:GetInflictor():GetClass() == "murwep_ragdoll_weapon" then
	...
	if phys_idx then
		...
		local shouldDismember = dismemberEnabled and gib_type_dismember and not (gib_type_bullet and hit_bone_name == "ValveBiped.Bip01_Head1") -- Don't dismember head on bullet damage
		...
		if gib_type_aoe then
			...
			if hurt_pos and dmginfo:GetDamage() >= 25 then
				self:ZippyGoreMod3_PhysBonesAOEDamage(hurt_pos, data)
			end
		end

		-- Direct and always gibtype:
		if (not gib_type_aoe and gib_type_direct) or gib_type_always then
			-- Direct damage, such as bullets:
			local data = {
				damage = gib_type_always and self.ZippyGoreMod3_PhysBoneHPs[phys_idx] or dmginfo:GetDamage(),
				forceVec = dmginfo:GetDamageForce(),
				dismember = shouldDismember,
			}
...
function ENT:ZippyGoreMod3_GuessBloodColorFromDamage(dmginfo)
	...
	if trEndPos then
		...
		for _, v in ipairs(ents.FindInSphere(trStartPos, trStartPos:Distance(trEndPos))) do
			if v ~= self then
				table.insert(filterEnts, v)
			end
		end
...
function ENT:ZippyGoreMod3_DamageRagdoll(dmginfo)
	if !IsValid(dmginfo) then return end
	local att = dmginfo:GetAttacker()
	if self.ZippyGoreMod3_BloodColor or self:ZippyGoreMod3_NewBloodColorOnDamage(dmginfo) then
...
function ENT:ZippyGoreMod3_PhysBonesAOEDamage(pos, data)
	...
	for i = 1, #phys_bones do
		...
		for k, v in ipairs(phys_bones) do
			if not mindist or v.dist < mindist then
				mindist = v.dist
				closest_phys_bone = v
			end
		end
	...
	for i, v in ipairs(phys_bones_sorted_dist) do
		local newData = {
			...
			dismember = data.dismember and (not data.explosion or (data.explosion and math.random(1, 2) == 1)),
		}
