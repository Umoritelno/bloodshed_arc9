...
function DMGINFO:ZippyGoreMod3_GibType()
	...
	if not (not ZGM3_CVARS["zippygore3_disable_never_gib_damage"] and self:IsDamageType(DMG_NEVERGIB)) then
		for dmg_type, type in pairs({
			...
			[DMG_ALWAYSGIB] = ZGM3_CVARS["zippygore3_disable_always_gib_damage"] and ZGM3_GIB_DIRECT or {ZGM3_GIB_ALWAYS, ZGM3_GIB_AOE},
		}) do
			if istable(type) then
				for _, v in ipairs(type) do
					if self:IsDamageType(dmg_type) and bit.band(gib_type, v) ~= v then
						gib_type = bit.bor(gib_type, v)
					end
				end
			elseif self:IsDamageType(dmg_type) and bit.band(gib_type, type) ~= type then
				gib_type = bit.bor(gib_type, type)
			end
		end
	end
...
function DMGINFO:ZippyGoreMod3_RagdollHitPhysBone(ent)
	...
	for i = 0, ent:GetPhysicsObjectCount() - 1 do
		...
		if ent.ZippyGoreMod3_PhysBoneHPs and ent.ZippyGoreMod3_PhysBoneHPs[i] == -1 then continue end
