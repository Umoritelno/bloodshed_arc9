...
function ENT:ZippyGoreMod3_CreateGib(pos, flesh_material, model, angles, scale, is_ragdoll, tangle_ragdoll)
	...
	return is_ragdoll and gib.Ragdoll or gib
end
...
function ENT:ZippyGoreMod3_CreateGibs(phys_idx, damageData)
	...
	local function get_custom_gibs(bone_name)
		if not ZippyGoreMod3_CustomGibs[bone_name] then return {}, 1 end
	...
	for _, v in ipairs(custom_gibs) do
		for i = 1, (v.count and (istable(v.count) and math.random(v.count[1], v.count[2])) or v.count) or 1 do
			local pos = (v.random_pos and get_bone_randompos()) or phys_bone:GetPos()
			local scale = v.scale and (istable(v.scale) and math.Rand(v.scale[1], v.scale[2])) or v.scale or 1
			local gib = self:ZippyGoreMod3_CreateGib(pos, v.use_flesh_material, v.model, not v.random_angle and phys_bone:GetAngles(), scale, v.is_ragdoll, true)
