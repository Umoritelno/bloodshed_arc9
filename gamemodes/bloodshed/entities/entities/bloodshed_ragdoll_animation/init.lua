...
function ENT:Initialize()
    ...
    timer.Simple(0.1, function()
        local bool = self.AnimGenerate
        local rag = self.Ragdoll
        if IsValid(rag) and bool then
            local physcount = rag:GetPhysicsObjectCount()
            for i = 0, physcount - 1 do
                if math.random(1,2) == 1 then 
                    local physObj = rag:GetPhysicsObjectNum(i)
                    physObj:SetContents(CONTENTS_MOVEABLE)
                end
            end
        end
    end)
...
function ENT:Think()
    ...
    if IsValid(rag) then
        ...
        for i = 0, physcount - 1 do
            ...
            if self.AnimGenerate and physObj:GetContents() == CONTENTS_MOVEABLE then continue end
